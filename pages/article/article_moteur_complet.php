...
while($data1 = mysqli_fetch_assoc($req5)) 
	...
	if($piece_nom <> $piece_nom_back)
		...
		$sql = ('SELECT * FROM piece_moteur WHERE piece = "'.$piece_nom.'" ');
		...
		while($data = mysqli_fetch_assoc($req)) 
			...
			if($data['niveau'] > $result3["$piece_nom"] AND $argent >= $data['prix'] AND $data['niv_exige'] <= $niveau)
				{
				if($data['act_turbo'] > '1' AND $act_turbo == '0')
					{
					echo '<div id="logo_3"><img onClick="javascript:alert(\'Votre véhicule ne possède pas de collecteur et de piston spécifique au montage turbo\')" border="0" src="images/achat/niv_'.$data['niv_exige'].'.png"></div></div></td>';
					}else if($data['act_turbo'] == '1' AND $act_turbo == '0'){
					echo '<div id="logo_3"><a href="index.php?page=new_achat&&type='.$piece_nom.'&&id='.$data['niveau'].'&&cat=moteur&&turbo='.$data['act_turbo'].'"><img border="0" src="images/achat/niv_'.$data['niv_exige'].'.png"></a></div></div>';
					}else if($data['act_turbo'] > '1' AND $act_turbo >= '1'){
					echo '<div id="logo_3"><a href="index.php?page=new_achat&&type='.$piece_nom.'&&id='.$data['niveau'].'&&cat=moteur&&turbo='.$data['act_turbo'].'"><img border="0" src="images/achat/niv_'.$data['niv_exige'].'.png"></a></div></div>';
					}else{
					echo '<div id="logo_3"><a href="index.php?page=new_achat&&type='.$piece_nom.'&&id='.$data['niveau'].'&&cat=moteur"><img border="0" src="images/achat/niv_'.$data['niv_exige'].'.png"></a></div></div>';
					}
				}else if($data['niv_exige'] > $niveau AND $data['act_turbo'] <= '1'){
				...
				}else if($argent < $data['prix'] AND $data['act_turbo'] <= '1'){
