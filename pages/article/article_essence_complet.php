<?php
$piece_nom_back = '';
$sql5 = ('SELECT * FROM piece_essence');
...
while($data1 = mysqli_fetch_assoc($req5)) 
	...
	if($piece_nom <> $piece_nom_back)
		...
		$sql = ('SELECT * FROM piece_essence WHERE piece = "'.$piece_nom.'" ');
		...
		while($data = mysqli_fetch_assoc($req)) 
			...
			if($data['niveau'] > $result3["$piece_nom"] AND $argent >= $data['prix'] AND $data['niv_exige'] <= $niveau)
				{
				echo '<div id="logo_3"><a href="index.php?page=new_achat&&type='.$piece_nom.'&&id='.$data['niveau'].'&&cat=essence"><img border="0" src="images/achat/niv_'.$data['niv_exige'].'.png"></a></div></div>';
